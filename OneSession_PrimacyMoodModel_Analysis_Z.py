...
Code 3- One Session Primacy Mood Model Analysis with Z-score normalization

The Primacy Mood Model, his implementation with Statistical analyses correlations (and regression model for betaR) for each condition in the one session experiment- with Z-score normalization. 
...
cols_to_norm = ['outcomeAmount', 'happySlider.response'] # Model input columns

for col in cols_to_norm:
    high_df[f"{col}_z"] = high_df.groupby('participant')[col].transform(lambda x: (x - x.mean()) / x.std()) # High Mood Target
    low_df[f"{col}_z"] = low_df.groupby('participant')[col].transform(lambda x: (x - x.mean()) / x.std()) # Low Mood Target
...
def model_results(model_data):
    ...
    for participant, group in grouped:
        ...
        mood = clean_group['happySlider.response_z'].values
        ...
        currentAmount = clean_group['outcomeAmount_z'].values
...
higt_output_path = os.path.join(DATA_DIR, "HighCon_ALLResults_z.csv")
...
low_output_path = os.path.join(DATA_DIR, "LowCon_ALLResults_z.csv")
LowCon_ALLResults.to_csv(low_output_path, index=False)

#%% Outlier detection
# Removing participants who are 3 standard deviations from the mean of betaP

mean_betaP = HighCon_ALLResults['betaP'].mean()
std_betaP = HighCon_ALLResults['betaP'].std()

threshold =  mean_betaP +3*std_betaP

HighCon_ALLResults = HighCon_ALLResults[HighCon_ALLResults['betaP'] < threshold]
